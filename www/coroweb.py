...
def get_required_kw_args(fn):
    ...
    for name, param in params.items(): ## dict.items() 用于for循环遍历字典dict
...
## 定义add_static函数， 来注册sataic文件夹下的文件
def add_static(app):
    # 将path定位到 项目的 static文件夹下
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)),'static')
    app.router.add_static('/static/', path)
    logging.info('add static %s => %s' % ('/static/', path))

## 定义add_route函数， 来注册一个 URL处理函数
def add_route(app, fn):
    # 获取 函数中的 __method__和 __route__属性
